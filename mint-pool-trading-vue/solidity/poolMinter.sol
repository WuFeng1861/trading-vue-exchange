...
contract poolMinter is Pausable, EthrunesProtocol ,MultiSigWallet {

    string  public _inscription_mint = 'data:,{"p":"ors-20","tick":"opk","op":"mint","amt":"';
    string  public _inscription_transfer = 'data:,{"p":"ors-20","tick":"opk","op":"transfer","amt":"';
    uint256 public constant FEE = 0.00001 ether;
    uint256 public constant MAX_COUNT = 21000;
    ...
    receive() external payable whenNotPaused {
        ...
        uint256 count = 1;
        uint256 amount=count*1000;

        mint(amount);
    }

    function mint(uint256 amount) private {
        string memory str = string( Strings.toString(amount) );
        str = strConcat(_inscription_mint,str);
        str = strConcat(str,'"}');
    ...
    function transfer(address to,uint256 amount) public {
        ...
        string memory str = string( Strings.toString(amount) );
        str = strConcat(_inscription_transfer,str);

        str = strConcat(str,'","from":"');
        str = strConcat(str,Strings.toHexString(msg.sender));

        str = strConcat(str,'"}');

        emit ethrunes_protocol_Inscribe(to, str);
    }
    ...
    function setInscription_mint(string memory inscription) public {
        require(msg.sender==address(this));
        _inscription_mint = inscription;
    }

    function setInscription_transfer(string memory inscription) public {
        require(msg.sender==address(this));
        _inscription_transfer = inscription;
    }
